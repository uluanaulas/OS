	...
	while (true) {
		if (!ReadFile(
			...
			if (chosen_option == 2) {
				...
				bool checker = WriteFile(hPipe, emp_to_push, sizeof(employee), &dwBytesWrite, NULL);
				if (checker) {
					std::cout << "Required Data was sent.\n";
				}
				else {
					std::cout << "Required Data wasn't sent.\n";
				}
				int msg;
				ReadFile(hPipe, &msg, sizeof(msg), &dwBytesWrite, NULL);
				if (msg == 1)
...
int main() {
	...
	for (int i = 0; i < number_of_employees; i++)
	{
		...
		std::cin >> emps[i].num;
		std::cout << "Input employee`s name:\n";
		std::string tmp;
		std::cin >> emps[i].name;
		std::cout << "Input employee`s hours:\n";
	...
	for (int i = 0; i < number_of_employees; i++)
	{
		...
		std::cout << "\nID of the employee: " << id << ".\nName of the employee: " << name << ".\nHours of the employee: " << hours << ".\n";

	}
	file_input.close();

	std::cout << "Input client`s number\n";
	...
	file_input.close();
	std::cout << "Click any key to exit...\n";
	_getch();
	return 0;
}
